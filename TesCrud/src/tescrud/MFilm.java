...
public class MFilm {
    ...
    public String[][] ReadFilm() {
        try {
            int jmlData = 0;
            String data[][] = new String[getBykData()][7];
            String query = "SELECT*FROM `rekapfilm`";
            ResultSet rs = statement.executeQuery(query);
            while (rs.next()) {
                data[jmlData][0] = rs.getString("id");
                data[jmlData][1] = rs.getString("judul");
                data[jmlData][2] = rs.getString("tipe");
                data[jmlData][3] = rs.getString("episode");
                data[jmlData][4] = rs.getString("genre");
                data[jmlData][5] = rs.getString("status");
                data[jmlData][6] = rs.getString("rating");
                jmlData++;
    ...
    public void DeleteFilm() {
        try {
            String query = "DELETE FROM `rekapfilm` WHERE id = ''";
            statement = (Statement) conn.createStatement();
            statement.executeUpdate(query);
    ...
    public void UpdateFilm(int id, String judul, String tipe, String episode, String genre, String status, String rating) {
        try {
